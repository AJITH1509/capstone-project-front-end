export const quotes = [
  {
    id: 1,
    text: "I like my money right where I can see it...hanging in my closet.",
    author: "Carrie Bradshaw",
  },
  {
    id: 2,
    text: "You can never be overdressed or overeducated.",
    author: "Oscar Wilde",
  },
  {
    id: 3,
    text: "Clothes mean nothing until someone lives in them.",
    author: "Marc Jacobs",
  },
  {
    id: 4,
    text: "Fashion is the armor to survive the reality of everyday life.",
    author: "Bill Cunningham",
  },
  {
    id: 5,
    text: "The dress must follow the body of a woman, not the body following the shape of the dress.",
    author: "Hubert de Givenchy",
  },
  {
    id: 6,
    text: "A girl should be two things: classy and fabulous.",
    author: "Coco Chanel",
  },
  {
    id: 7,
    text: "Style is a way to say who you are without having to speak.",
    author: "Rachel Zoe",
  },
  {
    id: 8,
    text: "I like my money where I can see it: hanging in my closet.",
    author: "Unknown",
  },
  {
    id: 9,
    text: "Fashion is not necessarily about labels. It's not about brands. It's about something else that comes from within you.",
    author: "Ralph Lauren",
  },
  {
    id: 10,
    text: "I always find beauty in things that are odd and imperfect - they are much more interesting.",
    author: "Marc Jacobs",
  },
  {
    id: 11,
    text: "I have always believed that fashion was not only to make women more beautiful, but also to reassure them, give them confidence.",
    author: "Yves Saint Laurent",
  },
  {
    id: 12,
    text: "The joy of dressing is an art.",
    author: "John Galliano",
  },
  {
    id: 13,
    text: "Fashion is a way of expressing yourself without having to say a word.",
    author: "Unknown",
  },
  {
    id: 14,
    text: "In difficult times, fashion is always outrageous.",
    author: "Elsa Schiaparelli",
  },
  {
    id: 15,
    text: "Elegance is not standing out, but being remembered.",
    author: "Giorgio Armani",
  },
  {
    id: 16,
    text: "I don't design clothes. I design dreams.",
    author: "Ralph Lauren",
  },
  {
    id: 17,
    text: "The dress is a beautiful way to express yourself.",
    author: "Unknown",
  },
  {
    id: 18,
    text: "I firmly believe that with the right footwear one can rule the world.",
    author: "Bette Midler",
  },
  {
    id: 19,
    text: "One is never over-dressed or underdressed with a Little Black Dress.",
    author: "unknown",
  },
];
